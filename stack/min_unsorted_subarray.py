...
def find_min_unsorted_subarray(integers: list[int] | tuple[int]):  # LeetCode Q.581.
    total_numbers = len(integers)
    next_smaller_indices = [-1] * total_numbers  # Indices of next smaller num.
    stack = [(0, integers[0])]  # Monotonic-increasing stack: (idx, num).

    for current_num_idx in range(1, total_numbers):
        if stack:
            past_idx, past_num = stack.pop(-1)
            while past_num > integers[current_num_idx]:  # Past num > current num: next smaller found.
    ...
    if sum(next_smaller_indices) == -total_numbers:
        return 0
    ...
    for iteration_idx, next_smaller_idx in enumerate(next_smaller_indices[::-1]):
        if next_smaller_idx != -1:
            last_misplaced_idx = total_numbers - 1 - iteration_idx
            break
    ...
    for iteration_idx, num in enumerate(integers[last_misplaced_idx + 1 - total_numbers:][::-1]):
