...
def collect_max_segment_sums(nums: list[int], queries: list[int]) -> list[int]:  # LeetCode Q.2382.
    total_nums = len(nums)
    ...
    ranges2sums = {f"0:{total_nums - 1}": prefix_sums[-1]}
    ...
    for removal_idx in queries:
        ...
        if removal_idx < total_nums - 1:  # A new right segment sum appears.
