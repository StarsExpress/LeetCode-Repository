...
def find_max_independent_set(nodes_dict: dict, query_nodes: list | tuple = None):
    ...
    nodes_list, iter_idx = list(nodes_dict.keys()), 2  # Main loop starts from 3rd node's index.
    ...
    if len(nodes_list) >= 3:
        while iter_idx != len(nodes_list):
            ...
            else:  # If two-before set doesn't retake lead, "use deep copy" to update its value to last set's.
