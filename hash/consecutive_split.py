...
def verify_consecutive_split(sorted_numbers: list[int]) -> bool:  # LeetCode Q.659.
    ...
    for number in sorted_numbers:
        # Incompletions can't become complete.
        if incompletions and list(incompletions.keys())[0] <= number - 2:
            return False

        if number - 1 in incompletions.keys():
            if incompletions[number - 1][1] > 0:  # Length: 1 to 2.
                incompletions[number - 1][1] -= 1
                if incompletions[number - 1][1] == incompletions[number - 1][2] == 0:
                    del incompletions[number - 1]

                if number not in incompletions.keys():  # Subseq max: number - 1 to number.
                    incompletions.update({number: {1: 0, 2: 0}})
                incompletions[number][2] += 1
                continue

            if incompletions[number - 1][2] > 0:  # Length: 2 to 3.
                incompletions[number - 1][2] -= 1
                if incompletions[number - 1][1] == incompletions[number - 1][2] == 0:
                    del incompletions[number - 1]

                if number not in completions.keys():  # Subseq max: number - 1 to number.
                    completions.update({number: 0})
                completions[number] += 1
                continue

        if number - 1 in completions.keys():  # Subseq max: number - 1 to number.
