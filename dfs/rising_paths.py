...
def count_rising_paths(self, grid: list[list[int]]) -> int:  # LeetCode Q.2328.
    self.grid, self.modulo = grid, 10 ** 9 + 7
    self.total_rows, self.total_cols = len(grid), len(grid[0])

    # Number of rising paths starting at each cell.
    self.rising_paths = [[None] * self.total_cols for _ in range(self.total_rows)]
