...
def find_frog_position(self, edges: list[list[int]], time: int, node: int) -> float:  # LeetCode Q.1377.
    ...
    self.visited_nodes, self.target_time_t_prob = dict(), 0
    self._dfs_probability(1, 0, 1)
    return self.target_time_t_prob

def _dfs_probability(self, node: int, time: int, probability: int) -> None:
