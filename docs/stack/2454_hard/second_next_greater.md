...
在整条数组玩二分查找或Ordered Set 都是想当然耳的$O(nlogn)$解方 这俩网站的作法最后固然是AC了
...
(小声说: Algo Monster和LeetCode Wiki给的解方 在C++耗费了350+ ms __把时间分布表往另一边拉大了__)
...
3. 续第1问 若我们有$n = 10^6$这么长的阵列 则下面哪种$k$值 最容易给所有元素出结果呢？
   (A) $k = 1$ (B) $k = 100$ (C) $k = 1000$ (D) $k = 1000000$

这三个小问题想明白了 就自然能掌握单调栈的哲学咯🤓
