...
int findClosest3Sum(vector<int> &nums, int target) // LeetCode Q.16.
{
    sort(nums.begin(), nums.end());

    int min_gap = numeric_limits<int>::max(), closest3Sum = 0;
    ...
    while (idx_3 > min_idx_2)
    {
        ...
        if (min_idx_2 >= 2)
        {
            ...
            if (gap < min_gap)
                min_gap = gap, closest3Sum = last_three_sum;
        }
            ...
            while (idx_1 >= 0)
            {
                if (idx_1 == 0 || nums[idx_1] + two_sum < target)
                {
                    ...
                    if (min(gap_1, gap_2) < min_gap)
                        if (gap_1 < gap_2)
                            min_gap = gap_1, closest3Sum = three_sum_1;

                        else
                            min_gap = gap_2, closest3Sum = three_sum_2;
