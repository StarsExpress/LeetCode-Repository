...
int mostBooked(int total_rooms, vector<vector<int>> &meetings) // LeetCode Q.2402.
{
    ...
    priority_queue<
        pair<long long, int>, vector<pair<long long, int>>, greater<pair<long long, int>>>
        occupied_rooms_heap;
    ...
    while (meeting_idx < meetings.size())
    {
        ...
        { // No available room: current meeting postpones.
            ...
            while (meeting_idx < meetings.size() && queue.size() < open_rooms_heap.size())
            {
                ...
                if (min_end_time > meetings[meeting_idx][0]) // Postponed.
        ...
        while (!queue.empty())
        {
            ...
            if (meetings_counts[room_num] == max_meetings && room_num < busiest_room)
                busiest_room = room_num; // Always take the smaller num.
        }
    }
